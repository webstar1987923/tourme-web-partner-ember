...
export default Ember.Component.extend({
  ...
  guideLanguages:Ember.computed('model.langs.length','newLangChange',function(){
    return this.get('model.langs')
  }),
  //Devuelve la lista de idiomas menos los que ya tiene asignados
  languages:Ember.computed('appConstants.languages', 'model.langs', 'newLangChange', function(){
    var languages = this.get('appConstants.languages');
    var modelLangs = this.get('model.langs');
  ...
  actions:{
    ...
    deleteLang(lang){
      this.set('model.langs', this.get('model.langs').reduce(function(clean, row){
    ...
    showEdit(show){
      ...
      if ( !show ) {
        ...
        if ( Ember.isNone(this.get('model.langs')) ) Ember.set(this.get('model'),'langs',[]);
      }
    },
    editGuide(){
      this.toggleProperty('newLangChange');
      this.sendAction('save',['langs']);
    },
  }
});
